...
useEffect(() => {
    fetch("https://api.spoonacular.com/recipes/" + selectedRecipe.id + "/information?includeNutrition=false&apiKey=" + ApiKey )
    .then(res => res.json())
    .then(recipe => {
...
return (
    ...
    <FlatList  
        ...
        renderItem={({item}) => (
        <Text>{item.name} {item.measures.metric.amount} {item.measures.metric.unitShort}</Text>
        )}
    />
